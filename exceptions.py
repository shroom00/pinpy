class PinterestError:
    ...
    def __init__(self, response) -> None:
        ...
        self.message_detail = (
            self.response_json["message_detail"]
            if "message_detail" in self.response_json
            else None
        )
        super().__init__(
            f"({self.api_code}) {self.api_message}{' - ' + self.message_detail if self.message_detail else ''}"
        )
